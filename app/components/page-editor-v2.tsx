...
interface PageEditorProps {
  ...
  /** The page's stored CSS — its hand-written custom part is applied in the canvas and preserved on save. */
  initialCss?: string;
  meta?: PageMeta;
...
/**
 * Separator between the page's hand-written custom CSS and the part the builder
 * regenerates on every save (compiled Tailwind, fonts, inline styles). The custom
 * CSS lives above it and is preserved verbatim; everything below is disposable.
 */
const GENERATED_CSS_MARKER =
  "/* ===== Auto-generated by the page builder — edits below are overwritten on save ===== */";

/** Extract the hand-written custom CSS (the portion above the generated marker). */
function customCssOf(css: string): string {
  const i = css.indexOf(GENERATED_CSS_MARKER);
  return (i === -1 ? css : css.slice(0, i)).trim();
}

type SidebarTab = "blocks" | "layers" | "properties" | "classes" | "page" | "icons" | "resources";

export function PageEditor({ projectData, defaultBodyClasses, initialDarkMode = false, initialCss = "", meta, onSave, saving = false }: PageEditorProps) {
  const [mounted, setMounted] = useState(false);
  // The page's hand-written custom CSS — applied live in the canvas and preserved
  // across saves (buildCompiledCss only regenerates Tailwind/fonts/inline styles).
  const [pageCss] = useState(() => customCssOf(initialCss));
  ...
  // Aggregated CSS of all components, injected into the canvas so their custom
  // styles render while editing (the live page gets this via the compiled CSS).
  ...
  useEffect(() => {
    fetch("/api/components")
      ...
      .then((data) => {
        if (data.components) {
          // Load full HTML + CSS for each component
          Promise.all(
              ...
              fetch(`/api/components/${c.slug}`)
                ...
                .then((d) => {
                  ...
                  return {
                    ...
                    css: (d.component?.css as string) ?? "",
                  };
                })
                .catch(() => null)
            )
          ).then((results) => {
            const ok = results.filter(Boolean) as { block: import("~/lib/page-builder").PBBlock; css: string }[];
            setCustomComponents(ok.map((r) => r.block));
            setComponentCss(ok.map((r) => r.css).filter(Boolean).join("\n"));
          });
        }
      })
      .catch(() => {});
  ...
  const handleSave = useCallback(() => {
    ...
    setSavingCss(true);
    buildCompiledCss(html, state.root, externalStyles).then((compiled) => {
      setSavingCss(false);
      // Keep the hand-written custom CSS verbatim; only the generated block is rebuilt.
      const css = [pageCss, GENERATED_CSS_MARKER, compiled].filter((s) => s.trim()).join("\n\n");
      onSave(projectJson, html, css, pageMeta);
    });
  }, [store, state.root, externalStyles, onSave, pageMeta, pageCss]);
    ...
    <div className="rounded-lg border overflow-hidden bg-background">
      ...
      <div className="flex" style={{ height: "calc(100vh - 200px)", minHeight: "600px" }}>
        ...
        <div className="flex-1 bg-white dark:bg-gray-950 overflow-auto">
          <Canvas store={store} externalStyles={externalStyles} componentCss={componentCss} pageCss={pageCss} initialDarkMode={initialDarkMode} />
        </div>
      </div>
    </div>
  );
